...
export function SoccerFieldLineup({ players, formation }: ISoccerFieldLineUp) {
  ...
  const orientation: OnFieldOrientation =
    mobileRangeFull || tabletRangeFull ? 'portrait' : 'landscape'
  ...
  // Container: proporção 2:3 em portrait, original em landscape
  const fieldContainerClass = mobileRangeFull
    ? 'w-[92%] mx-auto aspect-[2/3]'
    : tabletRangeFull
      ? 'w-[70%] mx-auto aspect-[2/3]'
      : 'h-svh w-[88%]'
        ...
        <div className='relative h-full w-full rounded-sm border border-white/20 overflow-hidden'>
          ...
          {isPortrait ? (
            <>
              ...
              <div className='absolute top-0 left-1/2 -translate-x-1/2 w-[75%] h-[22%] border border-white/20 flex justify-center items-start'>
                <div className='w-[55%] h-[50%] border border-white/20' />
              </div>
              ...
              {/* Círculo central */}
              <div className='absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[45%] aspect-square border border-white/20 rounded-full flex items-center justify-center'>
                <div className='w-2 h-2 bg-white/50 rounded-full' />
              </div>

              {/* Área inferior (goleiro) */}
              <div className='absolute bottom-0 left-1/2 -translate-x-1/2 w-[75%] h-[22%] border border-white/20 flex justify-center items-end'>
                <div className='w-[55%] h-[50%] border border-white/20' />
              </div>
            </>
          ) : (
          ...
          {positions.map((pos, index) => {
            ...
            return (
              <div
                key={player.number}
                ref={(el) => { if (el) playersRef.current[index] = el }}
              ...
              >
                ...
                <span className='text-xs text-white/70 tracking-wider opacity-0 translate-y-1 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200'>
                  {player.name}
                </span>
              </div>
            )
