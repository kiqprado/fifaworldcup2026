...
import {
  ...
  CardGroupQualifier,
  IGroup,
} from '@/app/components/card-group'
...
export default function GroupsClientPage({
  ...
  function DoesGroupMatchSearch(
    ...
    const normalizedGroupKey =
      NormalizeText(group.keyGroup)

    const normalizedGroupLabel =
      NormalizeText(
        `Grupo ${group.keyGroup}`,
      )

    const matchesGroup =
      normalizedGroupKey ===
        normalizedSearch ||
      normalizedGroupLabel.includes(
        normalizedSearch,
      )
  ...
  return (
    <div className="min-h-svh relative flex flex-col items-center">
      ...
      <div
        className={`
          flex flex-wrap justify-center gap-8 py-8
          ${
            mobileRangeFull
              ? 'px-6'
              : ''
